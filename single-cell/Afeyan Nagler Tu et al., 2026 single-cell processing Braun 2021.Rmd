...
exportdata.forfig$barcodes <- rownames(exportdata.forfig)
writexl::write_xlsx(exportdata.forfig, "xlsx_output/merged.RCC.all.clusterinfo.xlsx") #Supp. Table 5
```
...
#ED Fig. 2d and e
merged_RCC.harmony <- readRDS("objects/merged_RCC.NeoVax.CancerCell.rds")
tumor.cells <- subset(merged_RCC.harmony, subset = sampleorigin == "Tumor" & unified_cluster %in% c("RCC1", "RCC2", "RCC3"))

tumor.cells$TLSstatus <- ifelse(tumor.cells$patient %in% c("16097-101", "16097-102", "16097-104", "16097-106", "16097-107", "16097-108", "16097-112", "DBCC03", "DBCC05", "DBCC11", "DBCC15", "DBCC16"), "TLS+", "TLS-")

TLStumor.markers <- FindMarkers(tumor.cells, ident.1 = "TLS+", group.by = "TLSstatus")
TLStumor.markers$gene <- rownames(TLStumor.markers)
TLS.tumor.volcano <- EnhancedVolcano::EnhancedVolcano(TLStumor.markers, rownames(TLStumor.markers), x = "avg_log2FC", y = "p_val_adj", pCutoff = 0.01, FCcutoff = 3, shape = 20, colAlpha = 1, col = c('black','black','black','red'), gridlines.major = TRUE, gridlines.minor = TRUE, border = 'full')
ggsave(file= "Graphics/TLS.tumor.volcano.DEG.pdf", plot=TLS.tumor.volcano, width=8, height=8)
xlsx::write.xlsx(TLStumor.markers, "xlsx_output/TLS.tumorcell.markers.singlecell.xlsx", sheetName = "DEG.singlecell")

TLS.plus.genes <- list("TLS+.markers" = TLStumor.markers$gene[TLStumor.markers$p_val_adj < 0.01 & TLStumor.markers$avg_log2FC > 3], "TLS-.markers" = TLStumor.markers$gene[TLStumor.markers$p_val_adj < 0.01 & TLStumor.markers$avg_log2FC < -3])

tumor.cells <- AddModuleScore(tumor.cells, features = list(TLS.plus.genes[["TLS+.markers"]]), name = "TLS+.markers")
tumor.cells <- AddModuleScore(tumor.cells, features = list(TLS.plus.genes[["TLS-.markers"]]), name = "TLS-.markers")

FeaturePlot(tumor.cells, "TLS+.markers1", order = TRUE)
VlnPlot(tumor.cells, features = c("TLS+.markers1", "TLS-.markers1"), group.by = "patient", split.by = "TLSstatus")
VlnPlot(tumor.cells, features = c("TLS+.markers1", "TLS-.markers1"), group.by = "TLSstatus")

TLSmarker.score <- FetchData(tumor.cells, vars = c("patient", "TLSstatus", "TLS+.markers1", "TLS-.markers1"))

score_summary <- TLSmarker.score %>%
  group_by(patient, TLSstatus) %>%
  summarize(
    across(all_of(c("TLS+.markers1", "TLS-.markers1")),
           list(mean = ~mean(.x, na.rm = TRUE), sd = ~sd(.x, na.rm = TRUE)),
           .names = "{.col}_{.fn}"),
    .groups = "drop"
)

score_summary_tidy <- bind_rows(
  score_summary %>%
    transmute(patient, TLSstatus, signature = "TLS+",
              mean = `TLS+.markers1_mean`),
  score_summary %>%
    transmute(patient, TLSstatus, signature = "TLS-",
              mean = `TLS-.markers1_mean`)
)

write.xlsx(score_summary, "xlsx_output/TLS.single.cell.scores.bypt.xlsx")

### for a reviewer question about lymphotoxin expression
T.cells <- subset(merged_RCC.harmony, unified_cluster %in% c("CD8.TRM", "CD8.TEX", "TAP", "CD4.TMEM", "TPROL", "CD4.TREG", "CD8.IFN"))
VlnPlot(T.cells, features = c("LTA", "LTB"), group.by = "unified_cluster", cols = c("#0433FF", "#FF2F92",  "#FF5000", "#FF2600", "#0296FF", "#C0C0C0", "#FFD309"))
ggsave("Graphics/LTALTBexpression.Tcells.pdf", width = 8, height = 4)
```
